...
class SyncHinxtonLondon:
    ...
    def __init__(self, es_staging, es_fallback, es_production, logger):
        ...
        self.from_path = '/nfs/public/rw/reseq-info/elastic_search_staging/' \
                         'snapshot_repo/es6_faang_repo/'
        self.to_path = '/nfs/public/rw/reseq-info/elastic_search/' \
                       'snapshot_repo/es6_faang_repo/'
    ...
    def create_snapshot(self, rep_type):
        ...
        parameters = {
            "indices": "file,organism,specimen,dataset,experiment,"
                       "protocol_files,protocol_samples,summary_specimen,"
    ...
    def restore_snapshot(self):
        ...
        parameters = {
            ...
            "rename_pattern": "(protocol_files|protocol_samples|"
                              "faang_build_3_organism|faang_build_3_specimen"
                              "|faang_build_3_dataset|faang_build_3_file|"
                              "faang_build_3_experiment|summary_specimen"
                              "|summary_organism|summary_file|summary_dataset)",
    ...
    def change_aliases(self):
        ...
        actions = {"actions": [
            {"remove": {"index": "{}_faang_build_3_file".format(self.yesterday),
                        "alias": "file"}},
            {"add": {"index": "{}_faang_build_3_file".format(self.today),
                     "alias": "file"}},
            {"remove": {"index": "{}_faang_build_3_organism".format(
                self.yesterday), "alias": "organism"}},
            {"add": {"index": "{}_faang_build_3_organism".format(self.today),
                     "alias": "organism"}},
            {"remove": {"index": "{}_faang_build_3_specimen".format(
                self.yesterday), "alias": "specimen"}},
            {"add": {"index": "{}_faang_build_3_specimen".format(self.today),
                     "alias": "specimen"}},
            {"remove": {"index": "{}_faang_build_3_dataset".format(
                self.yesterday), "alias": "dataset"}},
            {"add": {"index": "{}_faang_build_3_dataset".format(self.today),
                     "alias": "dataset"}},
            {"remove": {"index": "{}_faang_build_3_experiment".format(
                self.yesterday), "alias": "experiment"}},
            {"add": {"index": "{}_faang_build_3_experiment".format(self.today),
                     "alias": "experiment"}},
            {"remove": {"index": "{}_protocol_files3".format(self.yesterday),
                        "alias": "protocol_files"}},
            {"add": {"index": "{}_protocol_files3".format(self.today),
                     "alias": "protocol_files"}},
            {"remove": {"index": "{}_protocol_samples3".format(self.yesterday),
                        "alias": "protocol_samples"}},
            {"add": {"index": "{}_protocol_samples3".format(self.today),
                     "alias": "protocol_samples"}},
            {"remove": {"index": "{}_summary_specimen".format(self.yesterday),
                        "alias": "summary_specimen"}},
            {"add": {"index": "{}_summary_specimen".format(self.today),
                     "alias": "summary_specimen"}},
            {"remove": {"index": "{}_summary_organism".format(self.yesterday),
                        "alias": "summary_organism"}},
            {"add": {"index": "{}_summary_organism".format(self.today),
                     "alias": "summary_organism"}},
            {"remove": {"index": "{}_summary_file".format(self.yesterday),
                        "alias": "summary_file"}},
            {"add": {"index": "{}_summary_file".format(self.today),
                     "alias": "summary_file"}},
            {"remove": {"index": "{}_summary_dataset".format(self.yesterday),
                        "alias": "summary_dataset"}},
            {"add": {"index": "{}_summary_dataset".format(self.today),
                     "alias": "summary_dataset"}}
        ]
        }
    ...
    def delete_old_indices(self):
        ...
        self.es_fallback.indices.delete(
            index=f"{self.yesterday}_faang_build_3_specimen," +
        ...
        self.es_production.indices.delete(
            index=f"{self.yesterday}_faang_build_3_specimen," +
                  f"{self.yesterday}_faang_build_3_file," +
                  f"{self.yesterday}_summary_dataset," +
                  f"{self.yesterday}_summary_specimen," +
