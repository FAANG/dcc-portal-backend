...
croak "Need -es_index e.g. faang_build_1 which will be the new target index of the alias" unless ( $es_index);

my $term;
open $term, "curl -XGET \'$es_host/_alias\'|";
my $json = decode_json(&readHandleIntoString($term));
my %current = %$json;
die "Could not find the specified index $es_index at server $es_host" unless (exists $current{$es_index});
my %in_use;
